...
save_kable(x, file = "maxent_bioclim_auc_iou.html")
webshot::webshot('C:/git/weather-station-optimization/maxent_bioclim_auc_iou.html', file = paste0(fig_dir, "maxent_bioclim_auc_iou.png"))

x <- auc_labels %>%
  filter(Location %in% loc.number[7:14]) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped"))
save_kable(x, file="maxent_bioclim_auc_fire_reg.html")
webshot::webshot('C:/git/weather-station-optimization/maxent_bioclim_auc_fire_reg.html',
                 file = paste0(fig_dir, 'maxent_bioclim_auc_fire_reg.png'))
