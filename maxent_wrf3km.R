...
### Input climate vars: WRF 3km projections
...
wrf_files <- list.files(wrf_out)[grepl("tiff$", list.files(wrf_out))]

## Stack list to one climate variable brick
wrf_brick <- stack(paste0(wrf_out, wrf_files))
...
for (l in 2:length(loc.number)) {
  for (t in 1:5 ) {
    ...
    wx.station.spdf <- SpatialPointsDataFrame(coords=xy, data=station.sub)   
    #now convert spatial points df to simple feature
    wx.station.sf <- st_as_sf(wx.station.spdf) %>%
      st_set_crs(st_crs(fr.sf))
    ...
    pdf(file=paste0(fig_dir, "wrf3km_var_import_",file.location.string,"_wxstathresh_",t, ".pdf"), width = 8, height = 6)
    plot(maxent.me, main = paste("Predictor Importance \n ",location.string))
    dev.off()
    
    # response curves
    pdf(file=paste0(fig_dir, "wrf3km_res_curves_",file.location.string,"_wxstathresh_",t, ".pdf"), width = 12, height = 8)
    ...
    # 
    pdf(file=paste0(fig_dir, "wrf3km_roc_",file.location.string,"_wxstathresh_",t, ".pdf"), width = 8, height = 6)
    plot(e3, 'ROC')
    ...
    pdf(file=paste0(fig_dir, "wrf3km_ss_",file.location.string,"_wxstathresh_",t, ".pdf"), width = 8, height = 6)
    ...
    CA_lcc <- spTransform(CA, wrf_proj)
    CA.counties_lcc <- spTransform(CA.counties, wrf_proj)
    ...
    fout = paste0(fig_dir, "wrf3km_ss_",file.location.string,"_wxstathresh_",t,'.pdf')
    
    a <- ggplot(data = wx.stations.within.sf, aes(x = Longitude, y = Latitude)) + 
      geom_polygon(data = CA_lcc, aes(x=long, y = lat, group = group), fill = 'grey70', color='black', size=1) +
      geom_tile(data=test_df, aes(x=x, y=y, fill=layer), alpha=0.8) + 
      scale_fill_continuous_sequential(palette = "Viridis")+
      geom_polygon(data = CA.counties_lcc, aes(x=long, y=lat, group=group), color="black",fill=NA, size=0.5, alpha=0, show.legend = TRUE)+
    ...
    # #### Section 8: Data dump ####
    # 
    fout = paste0(output_dir, "wrf3km_model_",file.location.string,"_wxstathresh_",t,'.RData')
    save(list=c("maxent.me", "maxent.pred"), file=fout)
    
    fout = paste0(output_dir, "wrf3km_prediction",file.location.string,"_wxstathresh_",t,'.RDS')
...
# write auc table
write.csv(auc_table, paste0(output_dir, "maxent_wrf3km_auc_values.csv"), row.names = F)

auc_table <- read_csv(paste0(output_dir, "maxent_wrf3km_auc_values.csv"))
...
save_kable(x, file=paste0(fig_dir, "maxent_wrf3km_auc_iou.png"))

x <- auc_labels %>%
  filter(Location %in% loc.number[7:14]) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped"))
save_kable(x, file=paste0(fig_dir, "maxent_wrf3km_auc_fire_reg.png"))
