...
# Ensure the data frame is a tibble
scores.merged.df <- as_tibble(scores.merged.df)

# Create a subset of the stations with just latitude and longitude
less.merged.df <- scores.merged.df %>%
  dplyr::select('Lat','Lon','extracted.vals','Proposed.Weather.Station','Community') %>%
  arrange(desc(extracted.vals)) %>%
  rename("Lat" = Lat, "Lon" = Lon, "Similarity" = extracted.vals, "Station Name" = Proposed.Weather.Station, "Community" = Community)
