setwd('/home/owen/research/wildfire/wrf/proc/')
setwd('/home/owen/research/wildfire/wrf/proc/')
...
varin <- paste0('WRF_grid.nc')   #sample file
...
setwd('/home/owen/research/wildfire/wrf/proc/')
wrf.base <- readRDS('Q2_historical_wrf_summary.RDS')
...
#Convert the data.frame to a sptaial data frame
coordinates(wrf.base) = ~xlon+xlat

#Convert irregular WRF grid to spatial pixels data frame
pixels <- SpatialPixelsDataFrame(wrf.base, tolerance = 0.916421, wrf.base@data)
...
#Grace here a logic test is needed - if all NA then skip
...
writeRaster(out.brick, filename='FileNAme.grd', format="raster", overwrite=TRUE,options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
...
#############################################################
#Grace: I can't remember if this code works
...
#############################
#Make a proper plot for presentation
############################
...
a
ggsave(file=fout, plot = a, device = "png", dpi = 300) 


shape.extent <- extent(region_shape)
fr <- st_rasterize(region_shape,st_crs(IQR.raster))

rasterize(fr)

region_layer <- st_crop(x=st_as_stars(IQR.raster), mask=fr, crop=TRUE)    #create a poly-raster
